...
const BasicList: React.FunctionComponent<BasicListProps> = (props: BasicListProps): JSX.Element => {
  ...
  useEffect(() => {
    axios.post('/user/vipInfoListAll')
  ...
  const updateValid =(id,status)=>{
    axios.post('/manage/validVipInfoApply', {
      id: id,
      flag: status
    })
    .then(function (response) {
...
const applyVIP=()=>{
  ...
  const submitManager = async () => {
    axios.post('/manage/applyVipInfo', {
      id: window.localStorage.getItem('id'),
      data:newInfoField.getValues()
    })
    .then(function (response) {
  ...
  const renderOptions =(id, valid, verify)=>{
    if(userType == 'manager'){
      ...
      return(
        <div>
          <Button type="primary" onClick={openJoinDialog}>参与VIP</Button>
          <Dialog
                visible={joinVisible}
                title="申请参与VIP"
                style={{ width: '500px' }}
